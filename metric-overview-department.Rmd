...
slot_util <- slot_view_tbl %>%
  ...
  group_by(CAMPUS, DEPARTMENT_ID, DEPARTMENT, PROVIDER, PROV_ID, slot_year, slot_month) %>%
  ...
  left_join(dep_mapping) %>%
  mutate(SPECIALTY_GRP = ifelse(SPECIALTY %in%
  ...
  relocate(DEPT_SPECIALTY_NAME, .before = DEPARTMENT) %>%
  relocate(DEPARTMENT_ID, .after = DEPARTMENT) %>%
  relocate(SPECIALTY_GRP, .before = DEPT_SPECIALTY_NAME) 
...
htmltools::browsable(
  tagList(
    div(tags$label("Summary by", `for` = "access-metrics-dept")),
    tags$select(
      id = "access-metrics-dept",
      onchange = "Reactable.setGroupBy('access-metrics-dept', this.value ? [this.value] : [])",
      tags$option("None", value = ""),
